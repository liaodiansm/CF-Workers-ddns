...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const resp = await fetch(`https://1.1.1.1/dns-query?name=${subdomain}&type=${type}`, {
      ...
      if (data.TotalCount > 0) {
        record = data.DomainRecords.Record[0];
        if (record.RR != name) {
          record = null;
        }
      ...
      if (record == null) {
        //新建子域名
        actionUrl = await signUrl(accessKey, secretKey, "AddDomainRecord", {
          DomainName: domain,
          RR: name,
          Type: type,
          Value: addr,
        ...
        //更新子域名解析
        actionUrl = await signUrl(accessKey, secretKey, "UpdateDomainRecord", {
